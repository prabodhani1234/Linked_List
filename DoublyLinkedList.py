...
class DoublyLinkedList:
    ...
    def doublyListLength(self):
        current = self.head  # making variable as current and head node assign it
    ...
    def printForwardList(self):
        if self.head is None:  # checking head node is none
            ...
            while current is not None:  # checking current node is not none
                print(current.getData())  # printing data of current node
                current = current.getNext()  # making next of current node as current
    ...
    def printBackwardList(self):
        if self.head is None:  # checking head node is none
            ...
            newNode = self.head
            while newNode.getNext() is not None:  # checking next of new node is not none
                newNode = newNode.getNext()  # making next of new node as new node
            while newNode is not None:  # checking new node is none
    ...
    def addNodeInPos(self, pos, data):
        ...
        if pos > self.length - 1 or pos < 0:
            ...
            newNode = Node(data)
            current = self.head
            for i in range(0, pos - 1):  # running since 0 to position mines 1
                current = current.getNext()  # equaling next of current node to current

            newNode.setPrev(current)  # equaling current node to previous of new node
            newNode.setNext(current.next)  # equaling next of current node to next of new node
            current.next.setPrev(newNode)  # equaling new node to previous of next in current node
            current.setNext(newNode)  # equaling new node to next of current node
